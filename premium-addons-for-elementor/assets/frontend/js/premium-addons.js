	...
	$(window).on("elementor/frontend/init", function () {
		...
		var PremiumProgressDotsHandler = function ($scope) {
			...
			function dotIncrement(inc) {
				...
				if (inc === numberOfTotalFill) dotWidth = fillPercent;

				$dot.find(".segment-inner").animate(
					{
						width: dotWidth + "%",
					},
					speed / numberOfCircles,
					function () {
						increment++;
		...
		var PremiumVideoBoxWidgetHandler = function ($scope, $) {
			var $videoBoxElement = $scope.find(".premium-video-box-container"),
				$videoListElement = $scope.find(
					".premium-video-box-playlist-container",
				),
				$videoContainer = $videoBoxElement.find(
					".premium-video-box-video-container",
				), //should be clicked
				$videoInnerContainer = $videoBoxElement.find(
					".premium-video-box-inner-wrap",
				),
				$videoImageContainer = $videoInnerContainer.find(
				...
				$lighboxContainer = $videoListElement.length
					? $videoListElement
					: $videoBoxElement,
				lightBox = $lighboxContainer.data("lightbox")
					? $lighboxContainer.data("lightbox")
					: false,
				video,
				vidSrc;
			...
			if (lightBox) {
				if ("prettyphoto" === lightBox.type) {
					$lighboxContainer
						.find(".premium-vid-lightbox-container[data-rel^='prettyPhoto']")
						.prettyPhoto(getPrettyPhotoSettings(lightBox.theme));
				}
				...
				$lighboxContainer
					...
					.on("click", function (e) {
						triggerLightbox(
							$(this).closest(".premium-video-box-container"),
							lightBox.type,
						);
					});
			} else {
				...
				if ($videoListElement.length) {
					...
					$videoContainer.each(function (index, item) {
						var vidSrc,
							$videoContainer = $(item),
							$videoBoxElement = $videoContainer.closest(
				...
				if ("yes" !== sticky || "yes" === stickyOnPlay) return;
			...
			function playVideo() {
				...
				if (stickyOnPlay === "yes") stickyOption();
			...
			function triggerLightbox($container, type) {
				if ("elementor" === type) {
					$container
						.find(".premium-video-box-video-container")
						.trigger("click");
				} else {
					$container
						.find(".premium-vid-lightbox-container[data-rel^='prettyPhoto']")
			...
			function stickyOption() {
				...
				closeBtn
					...
					.on("click.closetrigger", function (e) {
						e.stopPropagation();
						stickyWaypoint.unobserve($videoBoxElement[0]);

						$videoBoxElement.removeClass(
							"premium-video-box-sticky-apply premium-video-box-sticky-hide",
						);
				...
				function checkResize(stickyWaypoint) {
					...
					if (
						[stickyDesktop, stickyTablet, stickyMobile].includes(
							currentDeviceMode,
						)
					) {
				...
				$(document).on("premium_after_sticky_applied", function (e, $scope) {
					...
					if (0 !== infobar.length) {
						...
						if (
							$scope.hasClass("premium-video-sticky-center-left") ||
							$scope.hasClass("premium-video-sticky-center-right")
						) {
							infobarHeight = Math.ceil(infobarHeight / 2);
							$videoInnerContainer.css(
								"top",
								"calc( 50% - " + infobarHeight + "px )",
							);
						}

						if (
							$scope.hasClass("premium-video-sticky-bottom-left") ||
							$scope.hasClass("premium-video-sticky-bottom-right")
						) {
							if ("" !== stickyMargin) {
		...
		var PremiumGridWidgetHandler = ModuleHandler.extend({
			...
			getDefaultElements: function () {
				...
				elements.$loadMore = elements.$galleryElement
					.parent()
					.find(selectors.loadMore);
				elements.$loadMoreDiv = elements.$galleryElement
					.parent()
					.find(selectors.loadMoreDiv);
			...
			updateCounter: function () {
				...
				this.settings.counter =
					this.settings.counter + this.settings.imageToShow;
			},

			updateGrid: function (gradHeight, $isotopeGallery, $loadMoreDiv) {
				$.ajax({
					url: this.appendItems(
						this.settings.counter,
						gradHeight,
						$isotopeGallery,
					),
			...
			loadMore: function (gradHeight, $isotopeGallery) {
				...
				if (
					$galleryElement.find(".premium-gallery-item").length >
					...
					$galleryElement
						.parent()
						.on("click", ".premium-gallery-load-less", function () {
							_this.settings.counter =
								_this.settings.counter - _this.settings.imageToShow;
						});
					...
					$galleryElement
						...
						.on(
							"click",
							".premium-gallery-load-more-btn:not(.premium-gallery-load-less)",
							function () {
								_this.updateCounter();
				...
				$vidWrap.each(function (index, item) {
					...
					$(item)
						...
						.on("click", function () {
							...
							$this
								.find(
									"img, .pa-gallery-icons-caption-container, .pa-gallery-icons-wrapper",
								)
								.css("visibility", "hidden");

							if ("style3" !== _this.settings.skin)
								$this
									.find(".premium-gallery-caption")
									.css("visibility", "hidden");
			...
			run: function () {
				...
				if ("metro" === this.settings.layout) {
					...
					$(window).resize(function () {
						_this.onResize($isotopeGallery);
					});
				}

				var $isotopeGallery = $galleryElement.isotope(
					this.getIsoTopeSettings(),
				);
				...
				if ("yes" !== this.settings.light_box) this.lightBoxDisabled();
				...
				$filters.find("a").click(function (e) {
					...
					if (_this.settings.loadMore)
						_this.appendItems(
							_this.settings.minimum,
							gradHeight,
							$isotopeGallery,
						);
				...
				if ("default" === this.settings.lightbox_type)
					this.$element
						...
						.prettyPhoto(this.getPrettyPhotoSettings());
			},
		...
		var PremiumCountDownHandler = function ($scope) {
			...
			var $countDownElement = $scope.find(".premium-countdown"),
				...
				until =
					"evergreen" === timerType
						? settings.until.date.replace(/ /g, "T")
						: settings.until,
				layout = "",
			...
			if ("flipping" === settings.style) {
				...
				$countDownElement.find(".countdown").countdown({
					...
					id: $scope.data("id"),
					circleStrokeWidth: computedStyle.getPropertyValue(
						"--pa-countdown-stroke-width",
					),
				...
				if ($scope.hasClass("premium-countdown-block")) {
					var currentValueWidth = $countDown
						.find(".countdown-amount")
						.last()
						.outerWidth();

					$countDown
						.find(".countdown-period span")
						.css("width", currentValueWidth);
				}
				...
				if (settings.reset) {
					$countDownElement
						...
						.countdown("option", "until", new Date(until));
				}

				if ("featured" === settings.style) {
					var $targetUnit = $countDownElement.find(
						".countdown-section-" + settings.featuredUnit,
					);
			...
			if ("." === settings.separator) {
				$countDown
					.find(".countdown_separator span")
					.addClass("countdown-separator-circle")
					.text("");
			}

			if (diff < 0) triggerExpirationAction();
				...
				if ("default" === settings.event && "flipping" !== settings.style) {
					setTimeout(function () {
						if ("dash" === settings.changeTo) {
							...
							"done" === settings.changeTo &&
							$countDown.find(".countdown-show4").length > 0
						) {
							var characters = ["D", "O", "N", "E"];

							characters.map(function (char, index) {
								$countDown
									.find(".countdown-amount > span")
									.eq(index)
									.text(char);
							});
						}
		...
		var PremiumCarouselHandler = function ($scope, $) {
			...
			$carouselElem
				.find(".elementor-section-stretched")
				.removeClass("elementor-section-stretched");

			var $progressbar = $carouselElem.find(
				".premium-carousel-nav-progress-fill",
			);

			if ($carouselElem.find(".item-wrapper").length < 1) return;
			...
			function initCarouselTilt() {
				var reverse = settings.mouseTiltRev,
					...
					$tiltTarget = thumbnailSlider
						? $scope.find(".premium-carousel-thumbnail")
						: $scope.find(".premium-carousel-template.item-wrapper");
			...
			function addSlideContent() {
				$scope
					.find(".premium-carousel-template[data-template-src]")
					.each(function () {
			...
			$carouselElem
				...
				.slick(getSlickOptions(settings));

			if (settings.hasNavSlider) {
				var isVerticalNav =
					$scope.hasClass("pa-thumb-nav-pos-row") ||
					$scope.hasClass("pa-thumb-nav-pos-row-reverse");
				...
				var navOptions = {
					...
					prevArrow: $carouselElem
						.find(".premium-carousel-nav-arrow-prev")
						.html(),
					nextArrow: $carouselElem
						.find(".premium-carousel-nav-arrow-next")
						.html(),
					dots: false,
					focusOnSelect: true,
					...
					centerPadding:
						computedStyle.getPropertyValue("--pa-thumb-slider-center-padding") +
						"px",
					asNavFor: $carouselElem.find(".premium-carousel-inner"),
				};

				if (settings.arrowCustomPos) {
					navOptions.appendArrows = $carouselElem.find(
			...
			function getSlickOptions(settings) {
				...
				if (settings.hasNavSlider) {
					...
					Object.assign(options, {
						...
						centerMode: settings.centerMode,
						centerPadding:
							computedStyle.getPropertyValue("--pa-carousel-center-padding") +
							"px",
						dots: settings.dots,
						...
						customPaging: function () {
							var customDot = $carouselElem
								.find(".premium-carousel-nav-dot")
								.html();
							return customDot;
						},
					...
					if (settings.arrowCustomPos) {
						options.appendArrows = $carouselElem.find(
							".premium-carousel-arrows-wrapper",
						);
					}
				}
			...
			if (settings.carouselNavigation === "progressbar") {
				...
				$carouselElem
					.find(".premium-carousel-nav-progressbar-fill")
					.css(
						"transform",
						"translate3d(0px, 0px, 0px) scaleX(" + progress + ") scaleY(1)",
					);
			}
			...
			$scope
				.find(".premium-carousel-hidden")
				.removeClass("premium-carousel-hidden");
				...
				settings.navigation.map(function (item, index) {
					if (item) {
						$(item).on("click", function () {
							var currentActive = $carouselElem
								.find(".premium-carousel-inner")
		...
		var PremiumModalBoxHandler = function ($scope, $) {
			...
			function addModalBoxContent($scope) {
				var $modalTemplate = $scope
					.find(".premium-modalbox-template[data-template-src]")
					.first();
			...
			if (!isDismissible) {
				...
				$modalElem
					.find(
						".premium-modal-box-close-button-container, .premium-modal-box-modal-footer",
					)
					.hide();
			}
						...
						entries.forEach(function (entry) {
							if (entry.isIntersecting) {
								setTimeout(function () {
									$modal
										.css("opacity", "1")
										.addClass("animated " + $modal.data("modal-animation"));
								}, animationDelay * 1000);
		...
		var PremiumBlogHandler = ModuleHandler.extend({
			...
			setLayoutSettings: function () {
				...
				var layoutSettings = {
					...
					infinite:
						"yes" === settings.premium_blog_infinite_scroll ? true : false,
				...
				if (layoutSettings.carousel) {
					...
					layoutSettings.spacing = parseInt(
						settings.premium_blog_carousel_spacing,
					);
					layoutSettings.autoPlay =
						"yes" === settings.premium_blog_carousel_play;
					layoutSettings.arrows =
						"yes" === settings.premium_blog_carousel_arrows;
					layoutSettings.fade = "yes" === settings.premium_blog_carousel_fade;
					layoutSettings.center =
						"yes" === settings.premium_blog_carousel_center;
					layoutSettings.dots = "yes" === settings.premium_blog_carousel_dots;
					layoutSettings.overflowSlides = "yes" === settings.overflow_slides;
					layoutSettings.speed =
						"" !== settings.carousel_speed
							? parseInt(settings.carousel_speed)
							: 300;
					layoutSettings.autoplaySpeed =
						"" !== settings.premium_blog_carousel_autoplay_speed
							? parseInt(settings.premium_blog_carousel_autoplay_speed)
							: 5000;
			...
			buildMarqueeLayout: function () {
				...
				// Make it draggable.
				var isDraggable =
					"yes" === this.getElementSettings("marquee_draggable") &&
					!elementorFrontend.isEditMode();
				if (isDraggable) {
			...
			getSlickSettings: function () {
				...
				if ("default" !== settings.arrows_position) {
					carouselOptions.appendArrows = this.$element.find(
						".premium-carousel-arrows-wrapper",
					);
				}
		...
		var PremiumTeamMembersHandler = ModuleHandler.extend({
			...
			run: function () {
				...
				if (carousel) $multiplePersons.slick(this.getSlickSettings());
			},
		});
		...
		var PremiumTitleHandler = function ($scope, $) {
			...
			if ($style9.length) {
				$style9.each(function () {
					...
					elm
						.find(".premium-title-style9-letter")
		...
		var PremiumBulletListHandler = ModuleHandler.extend({
			...
			bindEvents: function () {
				...
				if (
					...
					this.$element.closest(".premium-mega-nav-item").length > 0
				)
					this.$element
						.closest(".premium-mega-nav-item")
						.find(".premium-menu-link")
						.on("click", function () {
				...
				if (
					!this.elements.$listItems.data("list-animation") &&
					settings.show_connector === "yes"
				) {
			...
			run: function () {
				...
				var devices = [
					...
					return ele != elementorFrontend.getCurrentDeviceMode();
				});
				...
				devices.map(function (device) {
					...
					$scope.removeClass(function (index, selector) {
						return (
							selector.match(
								new RegExp("(^|\\s)premium-" + device + "type\\S+", "g"),
							) || []
						).join(" ");
					});
				});
					...
					entries.forEach(function (entry) {
						if (entry.isIntersecting) {
							...
							setTimeout(function () {
								element
									.next(
										".premium-bullet-list-divider , .premium-bullet-list-divider-inline",
									)
									.css("opacity", "1");
								element
									.next(
										".premium-bullet-list-divider-inline , .premium-bullet-list-divider",
									)
									.addClass("animated " + $listItems.data("list-animation"));
			...
			handleAlignment: function () {
				...
				if ("flex-end" === listAlignment) {
					$element
						.find(".pa-has-text-bullet:not(.premium-bullet-list-wrapper-top)")
			...
			addRandomBadges: function () {
				...
				if (settings.rbadges_repeater.length < 1) return;

				var $currentList = $(".elementor-element-" + this.$element.data("id"));

				if (
					!$currentList.is(":visible") ||
					this.$element.hasClass("randomb-applied")
				)
					return;
				...
				randomBadges.forEach(function (badge, index) {
					if ("" != badge.rbadge_selector) {
						var notBadgedItems = $(badge.rbadge_selector)
						...
						var numOfApplies =
							Math.floor(
								Math.random() * (badge.rbadge_max - badge.rbadge_min + 1),
							) + badge.rbadge_min;
			...
			updateBulletConnectors: function () {
				...
				elements.$items.each((index, item) => {
					...
					const nextWrapper = nextItem.querySelector(
		...
		var PremiumButtonHandler = function ($scope, $) {
			...
			if (
				$btnGrow.length !== 0 &&
				$scope.hasClass("premium-mouse-detect-yes")
			) {
				$scope.on(
					"mouseenter mouseleave",
					".premium-button-style6",
			...
			if ($basicBtn.length) {
				...
				if (
					bgImage &&
					(bgImage.startsWith("linear-gradient") ||
						bgImage.startsWith("radial-gradient"))
				) {
		...
		var PremiumMaskHandler = function ($scope, $) {
			...
			if ("premium-addon-title.default" === $scope.data("widget_type")) {
				...
				"premium-textual-showcase.default" === $scope.data("widget_type")
			) {
		...
		var PremiumSVGDrawerHandler = ModuleHandler.extend({
			...
			run: function () {
				...
				if (!$scope.hasClass("premium-svg-animated-yes")) return;
				...
				if ("automatic" === scrollAction) {
					scrollTrigger =
						"custom" !== settings.animate_trigger
							? settings.animate_trigger
							: settings.animate_offset.size + "%";
				...
				var fromOrTo = !$scope.hasClass("premium-svg-anim-rev-yes")
					...
					startOrEndPoint =
						"from" === fromOrTo
							? settings.animate_start_point.size
							: settings.animate_end_point.size;
				...
				$paths.each(function (pathIndex, path) {
					...
					if ($scope.hasClass("premium-svg-sync-yes")) pathIndex = 0;
				...
				if ("yes" === settings.svg_fill) {
					...
					var fillSpeed = parseFloat(
						settings && settings.svg_fill_speed && settings.svg_fill_speed.size,
					);
				...
				if ("viewport" === scrollAction) {
					var controller = new ScrollMagic.Controller(),
						scene = new ScrollMagic.Scene({
							...
							duration: settings.draw_speed
								? settings.draw_speed.size * 1000
								: "150%",
						});
		...
		var PremiumWorldClockHandler = function ($scope, $) {
			var _ = luxon,
				id = $scope.data("id"),
				settings = $scope
					.find(".premium-world-clock__clock-wrapper")
					.data("settings"),
			...
			window["clockInterval-" + id] = setInterval(
				clockInit,
				inc,
				settings,
				$scope,
			...
			function clockInit(settings, $scope, id) {
				var isInHotspots =
					$(".elementor-element-" + id).closest(".premium-tooltipster-base")
						.length > 0;
				...
				if (isInHotspots) {
					...
					if (
						!window["clockNumbers-" + id] &&
						analogclocks.includes(settings.skin) &&
						settings.showClockNum
					) {
						drawClockNumbers($scope);
				...
				if (!time) {
					var htmlNotice =
						'<div class="premium-error-notice">This Is An Invalid Timezone Name. Please Enter a Valid Timezone Name</div>';
					$scope.find(".premium-world-clock__clock-wrapper").html(htmlNotice);
				}

				if (["skin-1", "skin-5", "skin-6", "skin-7"].includes(settings.skin)) {
					$scope
						.find(".premium-world-clock__hours")
						.css(
							"transform",
							"rotate(" + (time.hours * 30 + (time.minutes * 6) / 12) + "deg)",
						)
						.text("");
					$scope
						.find(".premium-world-clock__minutes")
						.css("transform", "rotate(" + time.minutes * 6 + "deg)")
						.text("");
					$scope
						.find(".premium-world-clock__seconds")
						.css("transform", "rotate(" + time.seconds * 6 + "deg)")
						.text("");
				...
				if (settings.date) {
					if ("skin-3" === settings.skin) {
						...
						settings.dateFormat.forEach(function (format) {
							if (
								time.date[format] !== undefined &&
								time.date[format] !== null
							) {
								var html =
									'<span class="premium-world-clock__date-segment">' +
									time.date[format] +
									"</span>";
				...
				if (settings.gmtOffset) {
					var offset =
						"Z" === settings.offsetFormat ? time.offset + "HRS" : time.offset;
			...
			function equalWidth(skin) {
				var width = 0,
					selector =
						"skin-3" === skin
							? ".premium-world-clock__hand:not(.premium-world-clock__seconds)"
							: ".premium-world-clock__hand";
		...
		var PremiumPostTickerHandler = function ($scope, $) {
			var timer = null,
				...
				settings = $scope
					.find(".premium-post-ticker__outer-wrapper")
			...
			if (
				$postsWrapper.find(".premium-post-ticker__post-wrapper").length <= 6
			) {
				$postsWrapper
					.find(".premium-post-ticker__post-wrapper")
					.map(function (index, post) {
			...
			if (settings.arrows) {
				$scope
					.find(".premium-post-ticker__arrows a")
					.on("click.paTickerNav", function () {
			...
			$scope
				.find(".premium-post-ticker__outer-wrapper")
				.removeClass("premium-post-ticker__hidden");
			...
			function getSlickSettings() {
				...
				if (closestTab.length > 0) {
					if (!closestTab.hasClass("content-current")) autoPlay = false;
				}
			...
			function resetAnimations() {
				...
				$slides.each(function (index, elem) {
					$(elem)
						.removeClass("animated " + settings.animation)
						.addClass("elementor-invisible");
				});
			}
			...
			function triggerAnimation() {
				$postsWrapper
					...
					.each(function (index, elem) {
						$(elem)
							.removeClass("elementor-invisible")
							.addClass(settings.animation + " animated");
					});
			}
		...
		var PremiumWeatherHandler = function ($scope, $) {
			...
			var forecastHeight = $scope
				...
				$forecastSlider =
					"layout-2" === settings.layout
						...
						: $scope.find(".premium-weather__hourly-forecast-wrapper"),
				forecastTabs = $scope.hasClass("premium-forecast-tabs-yes")
					? true
					: false,
				dailyForecastCarousel = $scope.hasClass("premium-forecast-carousel-yes")
					? true
					: false,
				dailyEqWidth =
					!forecastTabs &&
			...
			if ($forecastSlider.length) {
				$forecastSlider
					...
					.slick(getSlickSettings(settings, settings.layout, false));
			}

			if (dailyForecastCarousel) {
				var dailyCarouselSettings = $scope
					.find(".premium-weather__outer-wrapper")
					.data("pa-daily-settings");

				$scope
					.find(".premium-weather__forecast")
					.slick(getSlickSettings(dailyCarouselSettings, "", true));
			}
			...
			if (
				...
				$scope
					.find(
						".premium-weather__hourly-forecast-wrapper a.carousel-arrow, .premium-weather__extra-outer-wrapper a.carousel-arrow",
					)
					.on("click.paWeatherNav", function () {
						var $slider = $(this).closest(
							".premium-weather__hourly-forecast-wrapper, .premium-weather__extra-outer-wrapper",
						);
		...
		var PremiumTiktokHandler = function ($scope, $) {
			...
			// share links
			$scope
				.find(".premium-copy-link")
				.on("click.paTiktokCopyLink", function () {
			...
			if ("play" === settings.onClick) {
				...
				$scope
					.find(".premium-tiktok-feed__video-media")
					.on("click", function () {
		...
		var PremiumPinterestHandler = function ($scope, $) {
			...
			if (loadBoardPins) {
				...
				$scope
					.find(".premium-pinterest-feed__board-trigger")
					.on("click.paHidePins", function () {
						$boardWrapper.show();
			...
			if (!isBoardQuery) {
				...
				if (settings.loadMore && !settings.carousel) {
					...
					window.paHiddenPins = $scope.find(
						".premium-pinterest-feed__pin-outer-wrapper.premium-display-none",
					).length;

					$scope
						.find(".premium-pinterest-feed__load-more-btn")
						.on("click.paLoadMorePins", function (e) {
							var bookmark = window.paLoadMoreBookmark,
								count = settings.loadMoreCount;

							for (var i = 0; i < count; i++) {
								$scope
									.find(".premium-pinterest-feed__pin-outer-wrapper")
									.eq(bookmark + i)
									.show()
									.addClass("premium-pin-shown");
							}
			...
			if ("layout-2" === settings.pinLayout) {
				$scope
					.find(".premium-pinterest-feed__pin-meta-wrapper")
					.on("click.paPinTrigger", function (e) {
						if (e.target === this) {
			...
			$scope.find(".premium-copy-link").on("click.paCopyLink", function () {
				$scope
					.find(".premium-pinterest-share-menu")
					.css("visibility", "hidden");
			...
			$scope
				.find(".premium-pinterest-share-item:not(.premium-copy-link)")
				.on("click.paShare", function () {
					var link = $(this).data("pa-link");
					window.open(link, "popup", "width=600,height=600");
				});
		...
		var PremiumTestimonialsHandler = ModuleHandler.extend({
			...
			getDefaultElements: function () {
				...
				return {
					$multipleTestimonials: this.$element.find(
						selectors.multipleTestimonials,
					),
			...
			runEqualHeight: function () {
				...
				var maxHeight = Math.max.apply(null, heights);
				$testimonials
					.find(".premium-testimonial-content-wrapper")
			...
			run: function () {
				...
				if (carousel) {
					...
					if ("skin4" === skin) slickSettings.infinite = false;

					$multipleTestimonials.slick(slickSettings);

					if ("skin4" === skin) {
						var $skinCarousel = this.$element.find(
						...
						this.$element
							.find(".premium-testimonial__carousel-img")
							.on("click", function () {
					...
					if ("none" !== settings.arrows_lq_effect) {
						this.$element
							.find("a.slick-arrow")
							.addClass("premium-con-lq__" + settings.arrows_lq_effect);
					}
				}
		...
		var PremiumMobileMenuHandler = ModuleHandler.extend({
			...
			getSlickSettings: function () {
				...
				if (
					-1 !== currentDeviceMode.indexOf("mobile") &&
					"yes" !== settings.carousel_arrows_mobile
				) {
					prevArrow = "";
					nextArrow = "";
				} else if (
					-1 !== currentDeviceMode.indexOf("tablet") &&
					"yes" !== settings.carousel_arrows_tablet
				) {
					prevArrow = "";
					nextArrow = "";
				}
			...
			run: function () {
				...
				if (carousel) $list.slick(this.getSlickSettings());
			},
			...
			observeSections: function () {
				...
				var observerCallback = function (entries) {
					if (!self.isNotScrolling) {
						entries.forEach(function (entry) {
							var $item = self.elements.$items.filter(
		...
		var functionalHandlers = {
			...
			"premium-textual-showcase.default": [
			...
			if ("object" === typeof func) {
				$.each(func, function (index, handler) {
					elementorFrontend.hooks.addAction(
						"frontend/element_ready/" + elemName,
						handler,
					);
				});
			} else {
