(function ($) {
	...
	var PremiumNavMenuHandler = function ($scope, $) {
		 ...
		 */
		$scope
			.find(".premium-nav-widget-container")
			.css({ visibility: "inherit", opacity: "inherit" });
		...
		var $menuContainer = $scope.find(".premium-mobile-menu"),
			...
			$fullWidthItems = $scope.find(
				'.premium-nav-menu-container li[data-full-width="true"]',
			),
			stickyProps = {},
			refreshPos = false,
			stickyIndex = "stickyPos" + $scope.data("id"),
			stickyWidthIndex = "stickyWidth" + $scope.data("id"),
			$cachedStickyTarget = null,
			$cachedStickySpacerEl = null,
			headroomInstance = null,
			disablePageScroll = $scope.hasClass("premium-disable-scroll-yes")
				? true
				: false,
		...
		$scope.find("div[data-mega-content]").each(function (index, elem) {
			...
			if ($(targetElement).length > 0) {
				...
				$currentItem.append(
					$targetElement.clone(true).addClass("pa-cloned-element"),
				);
			}
		});
		...
		if (!isEditMode) {
			...
			$('div[data-menu-id="' + $scope.data("id") + '"]')
				.not(".pa-cloned-element")
				.remove();
		...
		$centeredItems.each(function (index, item) {
			$(item)
				.closest(".premium-nav-menu-item")
				.addClass("premium-mega-item-static");
		});
		...
		if ("dot" === settings.hoverEffect) {
			var $badgedItems = $scope.find(
				".premium-mega-content-container .premium-badge-dot, .premium-sub-menu .premium-badge-dot",
			);
		...
		if (settings.closeAfterClick) {
			$menuContainer
				...
				.on("click.paAfterClick", function () {
					...
					if (!hasChildern) {
						// close mobile menu
						if (
							"slide" === settings.mainLayout ||
							"slide" === settings.mobileLayout
						) {
		...
		if (!renderMobileMenu) {
			// those classes are used in both editor & frontend.
		...
		if (["hor", "ver"].includes(settings.mainLayout)) {
			if ("hover" === settings.submenuEvent) {
				$scope
					.find(".premium-nav-menu-item")
					.on("mouseenter.PaItemHover", function (e) {
						e.stopPropagation();
						...
						if ($(this).hasClass("premium-sub-menu-item"))
							$(this)
								.parents(".premium-nav-menu-item")
								.addClass("premium-item-hovered");
					});

				$scope.on("mouseleave.PaItemHover", function (e) {
					hoverTimeout = setTimeout(function () {
						$scope
							.find(".premium-item-hovered")
							.removeClass("premium-item-hovered");
					}, delay);
				});

				// we need to make sure that premium-item-hover is not removed when hovering over a sub/mega menu.
				$scope
					.find(".premium-sub-menu, .premium-mega-content-container")
					.on("mouseenter.PaItemHover", function (e) {
						e.stopPropagation();
				...
				var $trigger = $scope.find(
					".premium-nav-menu-container .premium-nav-menu-item.menu-item-has-children" +
						triggerSelector,
				);
		...
		$hamMenuCloser.on("click", function () {
			$scope
				.find(
					".premium-mobile-menu-outer-container, .premium-nav-slide-overlay",
				)
				.removeClass("premium-vertical-toggle-open");
			$("body").removeClass("premium-scroll-disabled");
		});
		...
		$menuToggler.on("click", function () {
			if (
				...
				$scope
					...
					.addClass("premium-vertical-toggle-open");
				...
				if (canBeClicked) {
					...
					if ($($menuContainer).hasClass("premium-active-menu")) {
						$scope
							.find(".premium-mobile-menu-container")
		...
		$menuContainer
			.find(".premium-nav-menu-item " + triggerSelector)
			.on("click", function (e) {
				var $this = $(this),
		...
		$(document).on("click.PaCloseMegaMenu", function (event) {
			var isTabsItem = $(event.target).closest(
					".premium-tabs-nav-list-item",
				).length,
				isWidgetContainer = $(event.target).closest(
			...
			if (!isWidgetContainer && !isTabsItem) {
				...
				if ("click" === settings.submenuEvent) {
					$scope
						.find(".premium-nav-menu-container .premium-item-hovered")
						.removeClass("premium-item-hovered");
				}
			}
		});
		...
		// vertical toggler.
		if (
			$scope.hasClass("premium-ver-toggle-yes") &&
			$scope.hasClass("premium-ver-click")
		) {
			$scope.find(".premium-ver-toggler").on("click", function () {
				$scope
					.find(".premium-nav-widget-container")
					.toggleClass("premium-ver-collapsed", 500);
			});
		}
		...
		function fullWidthContent($item) {
			...
			if (customSelector) {
				...
				$parentSec =
					!isContainer || $parentSec.length < 1
						? $scope.closest(".elementor-top-section")
						: $parentSec;
			}
			...
			$($item)
				.removeClass("premium-mega-item-static")
				.find(".premium-mega-content-container, > .premium-sub-menu")
				.css({
					width: width + "px",
					left: sectionLeft + "px",
				});
		...
		function checkBreakPoint(settings) {
			...
			if (settings.breakpoint >= $(window).outerWidth() && !isMobileMenu) {
				...
				settings.breakpoint < $(window).outerWidth() &&
				!isDesktopMenu
			) {
				...
				$scope
					.find(".premium-mobile-menu-container .premium-active-menu")
					.removeClass("premium-active-menu");
				$scope.removeClass("premium-hamburger-menu premium-ham-dropdown");
				$scope
					.find(".premium-vertical-toggle-open")
					.removeClass("premium-vertical-toggle-open");
				$scope.find(".premium-nav-default").removeClass("premium-nav-default");
		...
		function checkStickyEffect() {
			var isSticky =
				$scope.hasClass("premium-nav-sticky-yes") &&
				// settings.stickyOptions &&
				$("#" + settings.stickyOptions.targetId).length &&
				!settings.stickyOptions.disableOn.includes(
			...
			if (isSticky) {
				...
				stickyProps.spacerClass =
					"premium-sticky-spacer-" + $("#" + stickyProps.targetId).data("id");

				$cachedStickyTarget = $("#" + stickyProps.targetId);
				$cachedStickyTarget.addClass("premium-sticky-active");

				setStickyWidth(stickyProps);

				// Add spacer to save the sticky target space in the dom.
				if (0 === $("." + stickyProps.spacerClass).length) {
					$('<div class="' + stickyProps.spacerClass + '"></div>').insertBefore(
						$cachedStickyTarget,
					);
				}

				$cachedStickySpacerEl = $("." + stickyProps.spacerClass);

				$(window).on("load.PaStickyNav", applyStickyEffect);
				$(window).on("scroll.PaStickyNav", applyStickyEffect);
			} else {
				$(window).off("scroll.PaStickyNav load.PaStickyNav");
				if ($cachedStickySpacerEl) {
					$cachedStickySpacerEl.remove();
				}
				...
				if ($cachedStickyTarget) {
					$cachedStickyTarget
						.removeClass(
							"premium-sticky-parent premium-sticky-active premium-sticky-parent-" +
								$scope.data("id"),
						)
						.css({
							top: "unset",
		 ...
		 * Always calculate the width on each load/resize since the sticky element might have a dynamic width ( % / vw ) or might be affected by other responsive settings.
		 */
		function setStickyWidth(stickyProps) {
			// TODO: check if we can use the spacer's width directly instead.
			var currStickyWidth =
					stickyWidthIndex + elementorFrontend.getCurrentDeviceMode(),
				isSticky = $("#" + stickyProps.targetId).hasClass(
			...
			window[currStickyWidth] =
				$("#" + stickyProps.targetId).outerWidth() + "px";
		...
		function applyStickyEffect() {
			var adminBarHeight = elementorFrontend.elements.$wpAdminBar.height() || 0,
				scrollTop = $(window).scrollTop() + adminBarHeight,
				currStickyWidth =
					stickyWidthIndex + elementorFrontend.getCurrentDeviceMode();

			if (!window[stickyIndex] || refreshPos) {
				// save the offset
				window[stickyIndex] = $cachedStickySpacerEl.offset().top;
				refreshPos = false;
			}

			if (scrollTop >= window[stickyIndex]) {
				$cachedStickySpacerEl.css(
					"height",
					$cachedStickyTarget.outerHeight() + "px",
				);
				$cachedStickyTarget
					.addClass(
						"premium-sticky-parent premium-sticky-parent-" + $scope.data("id"),
					)
					.css({
						width: window[currStickyWidth],
						top: adminBarHeight,
				...
				$cachedStickySpacerEl.css("height", "0px");
				$cachedStickyTarget
					.removeClass(
						"premium-sticky-parent premium-sticky-parent-" + $scope.data("id"),
					)
			...
			if (stickyProps.onScroll) {
				var isParent = $cachedStickyTarget.hasClass("premium-sticky-parent");

				if (isParent && !headroomInstance) {
					$cachedStickyTarget.addClass("premium-sticky-scroll-yes");
					headroomInstance = new Headroom($cachedStickyTarget[0], {
						tolerance: 5,
						classes: {
							initial: "animated",
							pinned: "slideDown",
							unpinned: "slideUp",
							offset: {
								up: $cachedStickyTarget.outerHeight() + 150, // first time only.
							},
						},
					});

					headroomInstance.init();
				} else if (!isParent && headroomInstance) {
					// destroy the instance to remove the listeners and classes added by headroom.
					// This is done to make sure that on every re-init the sticky offset is calculated correctly according to the current position of the element in the DOM.
					headroomInstance.destroy();
					headroomInstance = null;
					$cachedStickyTarget.removeClass("premium-sticky-scroll-yes");
				}
			} else {
				$cachedStickyTarget
					.filter(".premium-sticky-parent")
					.removeClass("premium-sticky-scroll-yes");
			}
		}
		...
		function addRandBadges(badges) {
			var $menuContainer = ["hor", "ver"].includes(settings.mainLayout)
				? $scope.find(".premium-nav-menu-container")
				: $scope.find(".premium-mobile-menu-container");

			badges.forEach(function (badge) {
				var targetCount = $menuContainer.find(
					badge.selector + ":not(.has-pa-badge)",
				).length;
			...
			var $badge = getBadgeElement(badge),
				...
				hoverEffectClass =
					"" !== settings.hoverEffect
						? "premium-badge-" + settings.hoverEffect
						: "";
	...
	$(window).on("elementor/frontend/init", function () {
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/premium-nav-menu.default",
			PremiumNavMenuHandler,
		);
	});
})(jQuery);
