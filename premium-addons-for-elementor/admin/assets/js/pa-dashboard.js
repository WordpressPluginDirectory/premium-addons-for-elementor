(function ($) {
	...
	// Feature notices (.pa-notice + data-notice): core removes the element,
	// we persist the dismissal.
	$(document).on("click", ".notice.pa-notice .notice-dismiss", function () {
		var key = $(this).closest(".pa-notice").data("notice");

		if (!key) {
			return;
		}

		$.post(PaNoticeSettings.ajaxurl, {
	...
	$(function () {
		...
		if ($deactivationPopUp.length < 1) return;

		function deactivate() {
			$deactivationPopUp.addClass("hidden");

			var $link = $(
				'tr[data-slug="premium-addons-for-elementor"] .deactivate a',
			);
		...
		$document.on(
			"click",
			".pa-deactivation-popup .close, .pa-deactivation-popup .dashicons, .pa-deactivation-popup",
			function (event) {
		...
		$document.on(
			"change",
			'.pa-deactivation-popup input[name][type="radio"]',
			function () {
				var $this = $(this);
				...
				var $targetedMessage = $("p[data-" + name + '="' + value + '"]'),
					$relatedSections = $this
						.parents(".body")
						.find("section[data-" + name + "]"),
					$relatedMessages = $this
						.parents(".body")
						.find(
							"p[data-" + name + "]:not(p[data-" + name + '="' + value + '"])',
						);
		...
		$document.on(
			"keyup",
			".pa-deactivation-popup input[name], .pa-deactivation-popup textarea[name]",
			function (event) {
		...
		$document.on(
			"click",
			".pa-deactivation-popup button[data-action]",
			function (event) {
				var $this = $(this);
				...
				var $optionsWrappers = $this.parents(".body").find(".options-wrap"),
					$toggle = $optionsWrappers.find(
						'input[name][type="checkbox"]:checked, input[name][type="radio"]:checked',
					),
					$fields = $optionsWrappers
